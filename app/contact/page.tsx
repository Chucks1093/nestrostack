...
const EmailIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[18px] h-[18px] text-blue">
...
const inputCls = "w-full bg-[#f7f9ff] border border-line rounded-lg px-4 py-[0.8rem] text-ink font-body text-[0.93rem] font-light transition-[border-color,box-shadow] duration-200 outline-none appearance-none placeholder:text-ink-dim focus:border-blue focus:shadow-[0_0_0_3px_rgba(26,86,219,0.1)] focus:bg-white"
...
export default function ContactPage() {
  return (
    <>
      ...
      <div className="bg-white pt-36 pb-20 px-10 border-b border-line relative overflow-hidden max-md:pt-28 max-md:pb-12 max-md:px-6">
        ...
        <div className="max-w-[1120px] mx-auto relative z-[2]">
          ...
          <p className="text-[1rem] text-ink-muted max-w-[500px] mx-auto leading-[1.88] text-center">
            No pitch. No pressure. Just a real 30-minute conversation about your business, where it is, and where you want it to go.
          </p>
        </div>
      </div>

      {/* ── CONTACT SPLIT ── */}
      <div className="bg-surface border-t border-line px-10 pt-20 pb-28 max-md:px-6 max-md:pt-12 max-md:pb-20">
        <div className="grid grid-cols-[1fr_1.5fr] gap-12 max-w-[1120px] mx-auto items-start max-lg:grid-cols-1">

          {/* Left: info */}
          <Reveal className="flex flex-col gap-6 pt-2">
            <h2 className="font-display text-[2rem] font-semibold leading-[1.2]">
              We are genuinely <em className="text-blue">glad you are here.</em>
            </h2>
            {[
              { icon: <EmailIcon />, label: 'Email', content: <a href="mailto:contact@nestrostack.com" className="text-[0.93rem] text-ink no-underline hover:text-blue transition-colors duration-200">contact@nestrostack.com</a> },
              { icon: <PhoneIcon />, label: 'Phone', content: <a href="tel:+18302669617" className="text-[0.93rem] text-ink no-underline hover:text-blue transition-colors duration-200">+1 830-266-9617</a> },
              { icon: <ClockIcon />, label: 'Response Time', content: <span className="text-[0.93rem] text-ink">Within 24 hours</span> },
              ...
              <div key={label} className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-[0.72rem] text-ink-muted uppercase tracking-[0.09em] mb-[0.2rem]">{label}</p>
                  {content}
                </div>
              </div>
            ))}
          ...
          <Reveal delay="d1" className="bg-white rounded-2xl p-12 shadow-[0_8px_40px_rgba(0,0,0,0.14)] max-md:p-8">
            ...
            <div className="mb-5">
              ...
              <textarea placeholder="What would feel like a win for you in 90 days?" className={`${inputCls} resize-y min-h-[110px]`} />
            </div>
      ...
      {/* ── CLARITY SECTION ── */}
      <div className="bg-surface border-t border-line py-20 px-10 max-md:px-6">
        <Reveal className="max-w-[860px] mx-auto">
          ...
          <h3 className="font-display text-[clamp(1.8rem,3vw,2.5rem)] font-semibold leading-[1.2] mb-3">
            What happens on <em className="text-blue">the Clarity Call?</em>
          </h3>
          <p className="text-[0.95rem] text-ink-muted mb-10 leading-[1.8]">A focused 30 minutes where we map your world together.</p>
          <div className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(180px,1fr))]">
            {[
              'We listen to where you are and what is not working',
              'We find the 1 to 2 systems that would make the biggest difference',
              'We tell you exactly what we would build and why',
              'You leave with clarity — whether you work with us or not',
            ].map((text, i) => (
              <div key={i} className="bg-white border border-line rounded-xl p-6 hover:border-line-blue transition-colors duration-[250ms]">
                <div className="font-display text-[2rem] font-bold text-blue-line leading-none mb-3">
                  {String(i + 1).padStart(2, '0')}
                </div>
                <p className="text-[0.88rem] text-ink-muted leading-[1.75]">{text}</p>
              </div>
            ))}
          </div>
        </Reveal>
